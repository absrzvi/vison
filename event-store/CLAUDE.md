...
**Cursor pagination**: `GET /api/v1/events?after=<event_id>&limit=N` returns events with `(timestamp, event_id) > (ref.timestamp, ref.event_id)` in ascending order. `next_cursor` is the `event_id` of the last item on a full page. The cloud-backend polls this endpoint and advances its cursor. Do not change the response shape (`{data, count, journey_id, next_cursor}`) without updating the cloud-backend sync client.

**Edge-egress anonymisation** (privacy boundary): `GET /api/v1/events` is the ONLY path off the train (cloud-sync pulls it). Rows are redacted by `egress_privacy.anonymise_page` → `oebb_shared.events.anonymise_envelope` before serialising: `track_id` becomes a keyed HMAC token (`EVENT_STORE_ANONYMISE_KEY`, salted by `journey_id`), `bbox`/`camera_id` are dropped from the safety events, `RAMP_DEPLOYED.triggered_by_track_id` is blanked, and `ACCESSIBILITY_DETECTED` (GDPR Article 9) is withheld entirely. This makes the "anonymised at the edge" privacy claim true on the wire. **Full-fidelity payloads are kept locally and on the `/ws` fan-out** — only this read endpoint redacts, so on-train fusion/ledger/accessibility correlation is unaffected. The handler returns redacted plain dicts (`response_model=None`) and does NOT rebuild them as `EventModel`, because the strict payload schema would reject the dropped fields; the payload was already validated on ingest, and cloud-backend re-validates on its own ingest. **`next_cursor` is derived from the RAW DB page, before redaction** — a full page whose last row is a withheld event still advances the cursor, or cloud-sync would stall re-pulling the gap.
