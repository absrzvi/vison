...
> **Amendment 2026-06-14 (installed vs active — Winston):** Refines the count above. **Only one platform side opens at any stop**, so of the 24 door cameras **INSTALLED** (both sides — which side is route/station-dependent and not known ahead of time, so both must be physically present), only **12 are ACTIVE** at a given stop (2 doors/car × 6 cars, single side). The "continuous duty cycle" and "no door-open gating in PoC" rulings above are **unchanged** — the 12 active cameras still run free-running, ungated. What changes is **capacity sizing**, which must use the **active** count (~17 streams incl. gangway, not ~29) — see *Hailo-8 Capacity Budget*. This is not door-open gating; it is the structural fact that one side is dark every stop.
...
**Upgrade path:** OAuth2 / OIDC with ÖBB identity provider at fleet rollout. Architecture must not assume API key is permanent — no hardcoded key logic in business layer. **Update (E11-S1, 2026-06-14):** the first half of this path is now realised — self-contained JWT auth with role claims and an IdP-swappable verification seam; see **ADR-23**. The shared `X-API-Key` is superseded for routing by per-user JWTs.
...
- **GDPR (NFR6):** the only operator-identifying field persisted is `operator_id` (the `VITE_OPERATOR_ID` approximation from ADR-21; replaced by verified JWT identity in E11-S1+, see **ADR-23**). No name/email/face/bbox/passenger PII enters the audit log.
...
#### ADR-23: Self-Contained JWT Authentication (2026-06-14)

**Context:** ADR-7 made cloud-backend auth a single shared `X-API-Key` for the PoC and named "OAuth2/OIDC with ÖBB identity provider at fleet rollout" as the upgrade path, with the explicit constraint that "architecture must not assume API key is permanent — no hardcoded key logic in business layer." Epic 11 (Control Centre Admin & Identity) needs real per-user identity: the kill-switch (ADR-21/E10-S1) and the audit trail (ADR-22) both record an `operator_id` that was, until now, an unverified free-text/`VITE_OPERATOR_ID` approximation. E11-S1 realises the **first half** of ADR-7's upgrade path — token-based auth with role claims — without yet adopting an external IdP.

**Decision (story E11-S1):**

- **Self-contained JWT for the PoC.** cloud-backend issues *and* verifies its own HS256 tokens against a local `users` table (`user_id` uuid PK, `username` unique, bcrypt `password_hash`, `role ∈ {admin, operator}`, `is_active`; Alembic `0009`). `POST /api/v1/auth/login` (username+password → access token) and `GET /api/v1/auth/me` are the only new public/identity endpoints. **No external IdP** — Keycloak / ÖBB-OIDC SSO is Phase 2 (out of scope), see the upgrade path below.
- **The verification seam (the load-bearing decision).** Token *verification* is isolated in one private core `_verify_token(raw) -> CurrentUser` (`cloud_backend/api/auth.py`) that reads issuer, algorithm, and key from `Settings` **only** — never hardcoded at a call site. Two thin extractors feed it: `get_current_user` (Authorization header) and `get_current_user_from_query` (`?token=` for SSE). The 15 protected routers depend on `get_current_user`/`require_role(...)`, never on JWT internals. **Consequence:** swapping to an external IdP (RS256/JWKS) later means re-pointing `Settings` + the key source in `_verify_token` — *zero* route or extractor edits. This is the OIDC-swap guarantee, proven by an integration test (`test_seam_oidc_swap`).
- **bcrypt directly, not passlib.** passlib 1.7.4 (unmaintained since 2020) cannot read bcrypt ≥ 4.1's version and raises on hashing; E11-S1 uses the `bcrypt` package directly (explicit 72-byte truncation on hash and verify).
- **Fail-closed.** An empty/unset `JWT_SECRET` mints no token and verifies none (every protected route 401s), mirroring the `cc_admin_key` posture. `jwt.decode` is always called with an explicit `algorithms=[…]` allow-list (the `alg:none`/algorithm-confusion defense) and `issuer=` enforcement.
- **SSE auth via query param (D8).** The landside live stream is SSE (`/api/v1/alerts/stream`, ADR-20); a browser `EventSource` cannot set an `Authorization` header, so the stream authenticates via `?token=<jwt>` validated by the same verification core (second extractor). **Known limitation (accepted PoC, E11-S1 code-review Decision 2):** the full short-TTL access token rides in the URL query string, so it can land in reverse-proxy / uvicorn access logs, browser history, and `Referer`. Accepted for the PoC (TLS + 60-min TTL); the **deployment mitigation is to strip the `token` query param from the access-log format** (an nginx/uvicorn log-config concern, not app code). Phase-2 SSO may move to a cookie, a `Sec-WebSocket-Protocol`-style subprotocol, or a short-lived stream-scoped ticket.
- **Staged cutover, no central middleware.** Auth is attached per-router (`APIRouter(dependencies=[Security(...)])`), not via middleware. E11-S1 swaps the protected routers + the `/api/v1/health` per-route gate from `require_api_key` to `get_current_user` in one reviewed pass; the infra probes `/health/live` and `/health/ready` and `POST /auth/login` stay open. **One documented exception (code-review Decision 1):** the machine-to-machine ingest `POST /api/v1/events` stays on `require_api_key` — a human's short-TTL JWT is the wrong credential model for an unattended producer (heartbeats + the landside MQTT→REST alert path); migrating it to a service-account token is a follow-up story. `require_api_key`/`Settings.api_key` therefore remain live (ingest) and importable, fully retired only when E11-S3 re-keys `operator_preferences`, E11-S4 retires `X-Admin-Key`, and the ingest service-token lands.

**Upgrade path (Phase 2, out of scope):** an external IdP (self-hosted Keycloak or ÖBB's own OIDC tenant) replaces local token *issuance* and the login screen with a redirect; verification re-points to the IdP's JWKS/RS256 via `Settings`. Because verification is decoupled from issuance, this is a contained story (E11 epic "Out of scope"), not a rewrite — avoiding building the SSO integration twice (a self-hosted Keycloak would itself be replaced by ÖBB's IdP at fleet rollout).

**Migration:** `cloud-backend/migrations/versions/0009_users.py` — new table only, safe under concurrent reads. Unique index on `username`; role check constraint.

**Consumes/extends:** ADR-7 (realises its OIDC upgrade path's first half), ADR-6 (the JWT Phase-2 trigger language), ADR-10 (error envelope, reused for 401/403), ADR-20 (SSE transport — the `?token=` gate), ADR-21/ADR-22 (replaces their `VITE_OPERATOR_ID` operator-identity approximation with verified JWT identity in E11-S3+).

**Addendum (E11-S2, 2026-06-14) — mid-session liveness refines the seam, does not weaken it.** E11-S2 (user management) adds an `is_active` liveness check so deactivating a user invalidates their *existing* token mid-session (not only their next login). The check lives in the **extractors** (`get_current_user` / `get_current_user_from_query`), applied *after* `_verify_token` — never folded into the crypto core. The distinction is deliberate and load-bearing: `_verify_token` answers *authentication* ("is this token cryptographically valid under the configured issuer?") and stays issuer/key/algorithm-driven — the OIDC-swap guarantee above is unchanged; the liveness lookup answers *authorization* ("is this validly-identified principal still active here?") and is local by definition — it survives a Keycloak/OIDC swap unchanged (ÖBB still deactivates operators in our `users` table). Guarded by `test_seam_liveness_survives_verifier_swap` (a deactivated user is 401'd even with an externally-issued, still-valid token). **User-management audit** (E11-S2) is recorded in a dedicated local `user_audit` table (Alembic `0010`) + structured logs — NOT the cross-service event envelope: user-account actions are landside-local and never cross the onboard→landside egress/anonymise boundary the event bus exists for (so no `USER_*` EventType / `shared/` schema change / contract-test cascade).

**Addendum (E11-S4, 2026-06-15) — kill-switch X-Admin-Key retired; the "Staged cutover" plan is now fully executed except the ingest service-token.** E11-S4 swapped the alert-class kill-switch router (`routes/admin_alert_classes.py`) from the shared `X-Admin-Key` (`require_admin_key` / `Settings.cc_admin_key`) to `require_role("admin")` (JWT). This is the `X-Admin-Key` retirement the "Staged cutover" point above named: `cc_admin_key` had no remaining reader once `require_admin_key` was deleted, so the Setting was removed and `CC_ADMIN_KEY` dropped from `.env.example`. The audit actor is now `current_user.username` (the authenticated admin), not a client-supplied `actor_name` body field — `AdminActionBody` was deleted; the POST body is empty. The three-way audit (alert_class_state row + ALERT_CLASS_DISABLED/REENABLED event envelope + structured log) and the fan-out filter (`services/fanout_filter.py`) are unchanged. **No ADR decision changes** — this addendum records the planned terminal state arriving on schedule. The one remaining `require_api_key`/`Settings.api_key` user is the machine-to-machine ingest (the documented Decision-1 exception); replacing it with a per-producer service identity is the last step of the staged cutover (Phase-2, tracked E11S3-D4).
