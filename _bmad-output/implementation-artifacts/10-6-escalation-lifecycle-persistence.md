...
- **D3 — tag naming + taxonomy (matches SHIPPED UI).** Field name is **`action_tags`** everywhere (the live CC contract already ships `action_tags` — [escalations.js:58](../../control-centre/src/api/escalations.js); the epic's `outcome_tags` is superseded). The taxonomy is **already shipped and in operator use** — do NOT invent one. [EscalationDetail.jsx:8-15](../../control-centre/src/components/live/EscalationDetail.jsx) defines the picker: `Passenger assisted`, `Police alerted`, `Station notified`, `Conrad instructed`, `No action required`, `Other`. The backend enum MUST accept exactly these. **Store a canonical key, not the label:** `passenger_assisted`, `police_alerted`, `station_notified`, `conrad_instructed`, `no_action_required`, `other`. The acknowledge/resolve endpoint maps incoming labels → keys (a small constant dict, single source of truth in `cloud_backend/api/escalations.py`) so re-wording a UI label never silently re-buckets historical funnel data in 10-2. `422` on any value outside the six. **⚠ This six-tag set is the PoC taxonomy pending ÖBB confirmation** — if ÖBB adds/renames tags, update BOTH the UI `ACTION_TAGS` array and this backend enum + key map in lockstep; the canonical keys are the contract, the labels are display-only.
