...
## Deferred from: 11-4-admin-ui-alert-class-killswitch (2026-06-15)

- **E11S4-D5** — The E10-S3 critical-alert SOP + routing matrix (`_bmad-output/operational-procedures/critical-alert-sop.md`, `alert-routing-matrix.md`) document operating the alert-class kill-switch via `curl` with `X-Admin-Key`. After 11-4 that path 401s — the kill-switch is now operated from the Control Centre "Alert Classes" admin screen under an admin JWT role. The operational instructions are stale. NOT rewritten in 11-4 (out of code scope; the SOP is an ÖBB-facing artifact). Rewrite the kill-switch operation section (curl+X-Admin-Key → UI/JWT) at the Epic-11 close, alongside Freya's WDS UX pass over the admin/identity shell. [_bmad-output/operational-procedures/critical-alert-sop.md, alert-routing-matrix.md]
- **E11S4-W1** — `alert_class_state.disabled_by`/`enabled_by` now store `current_user.username` (a display string), not a stable `user_id`. If a user is renamed (no rename path today) the audit trail's actor would not retroactively update. The cross-service ALERT_CLASS_DISABLED/REENABLED event envelope also carries the username, not the UUID. Acceptable for the PoC (usernames are stable, ~3 operators); if a username-change path is ever added, decide whether the kill-switch audit should key by `user_id` with a username join for display. [cloud-backend/src/cloud_backend/routes/admin_alert_classes.py]

## Deferred from: code review of 11-4-admin-ui-alert-class-killswitch (2026-06-15)

- **E11S4-R1-CI** — Stale `.gitlab-ci.yml` `security:admin-key-grep` job (`:75-90`) + its masked GitLab `CC_ADMIN_KEY` variable are dead after the E11-S4 X-Admin-Key retirement. Harmless (the job self-skips when the var is unset and passes), but misleading dead CI. Remove the job + the masked variable. Out-of-scope for 11-4 (CI config is Tier-3 shared infra; the swap itself didn't touch it). Bundle into Epic-9 (container/infra hardening) or the Epic-11 close. [.gitlab-ci.yml:75-90]
- **E11S4-R1-DoubleAuth** — Router-level `dependencies=[Security(require_role("admin"))]` + the per-route `current: CurrentUser = Security(require_role("admin"))` on each POST build distinct `_checker` closures, so `assert_user_active`'s `SELECT is_active` runs twice per disable/enable. Correctness fine (idempotent read); one extra DB round-trip on an admin-rare path. PRE-EXISTING accepted pattern — `routes/users.py` (11-2, reviewed+done) does exactly the same at :35/:126/:184/:249. Not introduced by 11-4 and changing it here would deviate from the canonical admin pattern. If ever optimised, do it project-wide: a single module-level `_admin = require_role("admin")` reused as both the router dep and the handler param (so FastAPI's identity-keyed dependency cache dedups it). [cloud-backend/src/cloud_backend/routes/admin_alert_classes.py, routes/users.py]
