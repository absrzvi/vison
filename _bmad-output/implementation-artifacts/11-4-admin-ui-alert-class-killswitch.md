...
- [Source: routes/admin_alert_classes.py](../../cloud-backend/src/cloud_backend/routes/admin_alert_classes.py) — the swap target (router dep, `AdminActionBody`, the 6 `actor_name` sites — 2 upsert + 2 envelope + 2 log, the audit plumbing to preserve).
...
> **Code review — Round 1 (2026-06-15, FULL adversarial wire-replay).** Blind Hunter (diff-only) + Edge Case Hunter (REAL-wire replay on live Postgres — SUCCEEDED) + Acceptance Auditor (all 6 ACs / 5 Decisions / 4 Security Tests), then triage. **No BLOCKER, no HIGH, no VIOLATED AC.** The Edge Case Hunter empirically proved on a real DB with real tokens: operator→403 on all three endpoints, bogus/empty/case-variant role→403, actor-spoof ignored on BOTH disable AND enable, deleted-user `sub`→401 (not 500), non-UUID `sub`→401 (not 500), expired/tampered admin token→401, old `X-Admin-Key`→401, and — the retrospective's headline bug class — the fan-out filter suppresses a freshly-ingested alert of a just-disabled class with ZERO sleep (`invalidate()` survives the swap). The Acceptance Auditor confirmed all 6 ACs + 5 Decisions + 4 Security Tests SATISFIED in code (AC5 browser / AC6 gate-counts NVFD by nature, accepted on the dev record). 3 patches (all cosmetic/hardening — no behaviour fix needed), 2 defers (pre-existing/out-of-scope), 0 dismissed.

- [x] [Review][Patch] Misleading actor comment in the fan-out test — `test_killswitch_fanout.py:93` said the audit actor is `"synthetic-admin"`, but `auth_header(role="admin")` mints `username="tester"`; `"synthetic-admin"` is the seeded DB-row username, never the token claim. Flagged independently by Blind + Acceptance — the exact DB-row-vs-token-claim conflation the swap eliminates. FIXED: comment corrected to `"tester"` + a note that the actor value is exercised by test_killswitch_auth_swap.py, not here. [cloud-backend/tests/integration/test_killswitch_fanout.py:92-96]
- [x] [Review][Patch] File List omits `sprint-status.yaml` — FIXED: added `sprint-status.yaml` (+ this story file) to the Docs/planning line of the File List. [11-4 story File List]
- [x] [Review][Patch] "5 actor sites" undercount — FIXED: corrected to 6 references (2 upsert + 2 envelope + 2 log, consolidated via one `actor = current.username` local per handler) at all four prose occurrences; also corrected a stale "FastAPI dedupes the dependency" claim in the Completion Notes (it does NOT — distinct `_checker` closures; tracked E11S4-R1-DoubleAuth). [11-4 story prose]
- [x] [Review][Patch] Promote the auth-swap security envelope to permanent regression tests (hardening) — FIXED: added 4 parametrized real-wire tests to test_killswitch_auth_swap.py — operator→403 on all three endpoints (disable/enable/GET), actor-spoof ignored on ENABLE (the disable path was already covered), and malformed-`sub`→401-not-500 for both a valid-but-absent UUID and a non-UUID subject. [cloud-backend/tests/integration/test_killswitch_auth_swap.py]
- [x] [Review][Defer] Stale `.gitlab-ci.yml` `security:admin-key-grep` job — the `cc_admin_key`/`CC_ADMIN_KEY` retirement leaves this CI secret-hygiene job + its masked GitLab variable dead. Harmless (self-skips when the var is unset → passes). Outside this story's touched-files scope (CI config is Tier-3 shared infra). [.gitlab-ci.yml:75-90] — deferred, out-of-scope (CI config), tracked in deferred-work.md.
- [x] [Review][Defer] Double `require_role("admin")` evaluation per POST — router-level + per-route `Security(require_role("admin"))` each build a distinct `_checker`, so `assert_user_active`'s `SELECT is_active` runs twice per disable/enable. Correctness fine (idempotent read); one extra round-trip on an admin-rare path. Pre-existing accepted pattern (identical in the reviewed-and-done `routes/users.py:35,126,184,249`) — changing it here would deviate from the canonical admin pattern. [cloud-backend/src/cloud_backend/routes/admin_alert_classes.py:45,~120,~155] — deferred, pre-existing (matches users.py), tracked in deferred-work.md.

**Round-1 outcome: APPROVED → DONE.** All 4 patches applied + verified; 2 defers tracked (E11S4-R1-CI, E11S4-R1-DoubleAuth); 0 dismissed; security-sentinel APPROVED (posture IMPROVED — shared admin secret retired, body-actor spoof vector closed). POST-PATCH GATES GREEN: backend **282 passed** (unit + integration on real Postgres — the 4 new regression tests RAN and passed on the real wire: operator→403 on all three endpoints, actor-spoof ignored on enable, malformed-`sub`→401-not-500 ×2; the INTEGRATION-MUST-RUN gate is satisfied, not deferred), mypy --strict clean (40 files), ruff clean (src + tests); frontend 288 vitest, eslint clean on touched. No behaviour fix was needed — every patch was cosmetic (test comment, File List, site-count, stale dedup claim) or hardening (the regression net). The Edge Case Hunter's real-wire replay + the dev-time A1 run had already proven the swap end-to-end; the new tests permanently pin that envelope.
...
- **Single clean swap, as the story predicted.** Router-level dep `require_admin_key` → `require_role("admin")`; the two POSTs take a per-route `current: CurrentUser = Security(require_role("admin"))` so the handler reads the actor; `AdminActionBody` + `require_admin_key` + `_admin_header` + the `secrets`/`APIKeyHeader` imports deleted. `body.actor_name` → `current.username` at all 6 references (2 upsert + 2 envelope + 2 log), consolidated via one `actor = current.username` local per handler. Fan-out filter, `invalidate()`, the audit envelope, and the GET shape untouched. (Note: the router-level and per-route `require_role("admin")` are distinct `_checker` closures, so FastAPI does NOT dedupe them — the liveness SELECT runs twice per POST; harmless idempotent read, identical to the canonical `routes/users.py` admin pattern — tracked E11S4-R1-DoubleAuth.)
...
**Docs/planning:** `_bmad-output/planning-artifacts/architecture.md` (ADR-23 E11-S4 freshness addendum), `_bmad-output/implementation-artifacts/deferred-work.md` (E11S4-D5 SOP rewrite + E11S4-W1 username-vs-uuid actor), `_bmad-output/implementation-artifacts/sprint-status.yaml` (status transitions ready-for-dev → in-progress → review), `_bmad-output/implementation-artifacts/11-4-admin-ui-alert-class-killswitch.md` (this story file)
