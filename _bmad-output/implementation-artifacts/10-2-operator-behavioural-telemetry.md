...
- [x] [Review][Patch] Median/p95 parity gap between the funnel route and the report — **ADDRESSED:** the funnel route ([escalations_audit.py:94-99](../../cloud-backend/src/cloud_backend/routes/escalations_audit.py)) computed both `PERCENTILE_CONT(0.5)` and `(0.95)`, but `services/alert_effectiveness_report.py:_funnel_rows` computed only the median. Added the matching `PERCENTILE_CONT(0.95) … AS p95_ack` (same `GREATEST(…, 0)` clamp), surfaced as a "P95 ack" column in the "Median ack latency by alert class" table (Retune-candidates table left median-only — p95 doesn't aid that ranking). Pinned by new `test_report_renders_median_and_p95_ack_latency` (latencies 10/20/30/40/50s → median 30s, p95 48s render distinctly). [alert_effectiveness_report.py:30,76-81,98-99,196-205]
