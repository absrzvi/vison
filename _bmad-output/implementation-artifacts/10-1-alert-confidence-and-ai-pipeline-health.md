...
## Senior Developer Review (AI) — Round 3 (chunks 2-4: fusion / cloud-backend / control-centre, 2026-06-13)

<!-- Scope: the original feature commit 80958e2 (4665 lines) split into the 3 chunks the R1 plan never reached (R1 covered chunk 1/4 only). Multi-agent workflow (wf_96fe254e-e12): each chunk → Blind Hunter + Edge Case Hunter + Acceptance Auditor in parallel → per-finding adversarial verification (independent skeptic, default-refute) → per-chunk synthesis. 82 agents, ~7.1M tokens. 73 raw findings → 11 confirmed by the verifier → reviewer re-verified each against LIVE HEAD source (caught 2 verifier false-positives on stale-diff CSS). No Blocker/High survived verification. Net: 1 patch, 3 decision-needed, 2 defer, the rest dismissed. -->

### Decision-needed

- [ ] [Review][Decision] **(clou-2) `GET /api/v1/health` (degraded flag) mounts with NO auth, unlike every sibling** — `health.py:13` is `APIRouter()` with no dependency; `health.py:66` serves `/api/v1/health` open. Every other `/api/v1/*` router is `Security(require_api_key)`-protected — including `ai_pipeline.py:14`, which mounts the **same `/api/v1/health` prefix** WITH auth (so two routers share a prefix with opposite postures). The dev Completion Note claims "open, same posture as /health/*", but the real CC-facing posture is authenticated. Confirmed against live HEAD + the story's own `test_health_degraded_flag.py:46` asserts 200 with no header. **Add `require_api_key` to match siblings, or accept open for PoC?** [cloud-backend/src/cloud_backend/routes/health.py:13,66]
- [ ] [Review][Decision] **(clou-1) Kill-switch compares onboard-clock `t_raised` vs server-clock `disabled_at`** — `is_filtered` filters only when `t_raised > disabled_at` (`fanout_filter.py`); `disabled_at` is server `NOW()` (`admin_alert_classes.py`) but `t_raised` is the onboard envelope timestamp (`ingest.py` sets both `timestamp`+`source_timestamp` from `ev.timestamp`; no server receive-time persisted). A train clock running slow lets a genuinely post-disable alert (`t_raised <= disabled_at`) LEAK through the kill-switch; running fast over-suppresses an in-flight escalation. Verified against live HEAD. **Persist a server receive-time column and filter on that, or accept clock-skew window for PoC (NTP-synced fleet assumption)?** [cloud-backend: fanout_filter.py · ingest.py · admin_alert_classes.py]
- [ ] [Review][Decision] **(clou-3+clou-4 merged) Unlocked check-await-set on the two singleton caches** — `_DegradedCache.get` (`health.py`) and `AlertClassFilter._load` (`fanout_filter.py`) both read a TTL timestamp → `await db.execute(...)` → write singleton state, with no `asyncio.Lock`. Two concurrent requests can interleave around the await (lost update / brief thundering herd); an admin `invalidate()` landing mid-`_load` can be overwritten by the older in-flight load, re-caching a pre-disable map for up to one TTL. Verified: mechanism real, impact bounded (no data loss; DB rows authoritative; worst case is ≤60s stale kill-switch on one worker). **Guard both with an `asyncio.Lock`, or accept the ≤TTL staleness window for PoC?** [cloud-backend/src/cloud_backend/routes/health.py · services/fanout_filter.py]

### Patch (unambiguous fix) — APPLIED + VERIFIED (2026-06-13)

- [x] [Review][Patch] **(fusi-1) Door-obstruction + slip-fall alert handlers can 500 instead of 202 on a payload `ValidationError`** — FIXED: widened both handler catches in `health.py` to `except Exception ... # Pattern 3: handler must never raise` (matching the wagon/occupancy handlers' established pattern). Added RED-verified regression test `test_door_obstruction_payload_validation_error_returns_202` (single-colliding-key model_versions → fused `len<2` ValidationError; confirmed it 500s WITHOUT the fix, 202 WITH). [fusion/src/fusion/health.py · tests/unit/test_health.py]
- [x] [Review][Patch] **(R2 follow-up, self-caught) `ledger.py` mypy --strict regression from R2 track_id/camera_id widening** — my R2 patches widened `WagonExit/EntryPayload.track_id` → `int | str` and `camera_id` → Optional; `fusion/ledger.py` consumes both as `int`/`str` (timer arg, dict key, `_PendingExit`), producing 5 `mypy --strict` errors. **R2 verification ran `fusion pytest` but NOT `mypy --strict`, so this slipped — gap noted.** FIXED: narrowed both fields once at each WAGON handler entry in `ledger.py` (fusion is on-train, upstream of egress — track_id is always the raw int, camera_id always present; a redacted event reaching fusion is a wiring bug, logged + skipped). fusion mypy --strict now clean; inference + event-store mypy confirmed unaffected (no int-consumption of WAGON ids). [fusion/src/fusion/ledger.py:on_wagon_exit/on_wagon_entry]

  **Verification:** fusion 156 passed, cov 93.39% (gate ≥90), mypy --strict clean, ruff clean. inference + event-store mypy clean.

  (fusi-1 root cause: `door_obstruction.py` builds `model_versions={**upstream, "door_sensor_firmware": ctx...}`; a crafted `DoorObstructionPayload.model_versions == {"door_sensor_firmware": X}` collapses to 1 entry → `AlertRaisedPayload` fused-basis `len>=2` validator raises inside `emit_alert` before any POST; the handler caught only `httpx.HTTPError` → FastAPI 500. Untrusted external boundary per root CLAUDE.md; the fix adopts the same `except Exception` Pattern-3 guard the wagon/occupancy handlers already use.)

### Defer

- [x] [Review][Defer] **(fusi-2) `door_firmware_version` accepts empty string** — a `/context` push with `door_firmware_version=""` overwrites the `"unknown"` default (`context_state.py` writes when `is not None`; `models.py` field has no `min_length`) and stamps `"door_sensor_firmware": ""` into fused provenance — passes the `len>=2` validator (key present) but records empty, non-auditable provenance. SNMP firmware OIDs can legitimately return "". Low impact (one audit field; alert never dropped). [fusion/src/fusion/context_state.py · models.py] — deferred, hardening: treat "" like None (or `min_length=1`).

### Dismissed (verifier-confirmed false positives + stale-diff artifacts) — 7

- **(cont-2 + cont-4) Amber AI-pipeline dot uses non-existent `--obb-sev-warning` token** — **FALSE POSITIVE on live HEAD.** The workflow agents read the feature-commit snapshot; on current HEAD `SystemHealth.css:145` already reads `.sh-sev-dot--amber { background: var(--obb-sev-medium); }` (the correct token), and `var(--obb-sev-warning)` appears in **0** files. No defect; nothing to patch. (Caught by reviewer re-verification against HEAD — the verifier missed the intervening fix.)
- **(cont-1) Model-details disclosure toggles on Enter but not Space** — a11y nicety on a `role="button"` div; real but Low and outside this story's a11y scope. Noted, not actioned.
- **(cont-3) AIPipelineRow one-shot fetch, no refresh** — fleet_state frozen at mount while drawer relative-times advance; sibling DegradedBanner polls 60s. Real UX staleness but a design choice for the System Health tab; not a correctness defect. (Candidate for Epic 8 analytics-hardening if desired.)
- 62 further raw findings were refuted by the adversarial verifier (plausible-but-wrong / handled-elsewhere / documented-intent) and dropped before synthesis.
