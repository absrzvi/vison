...
- [x] [Review][Decision] **(clou-2) `GET /api/v1/health` no auth** → **RESOLVED: add per-route auth.** Added `Security(require_api_key)` to the `/api/v1/health` route only (infra probes `/health/live` + `/health/ready` stay open for k8s/LB). `test_health_degraded_flag.py` updated to send `X-API-Key`; new `test_api_health_requires_api_key` (missing + wrong key → 401). [cloud-backend/src/cloud_backend/routes/health.py:66]
- [x] [Review][Decision] **(clou-1) Kill-switch clock-skew (onboard `t_raised` vs server `disabled_at`)** → **RESOLVED: accept for PoC (NTP assumption).** The fleet is NTP-synced and the kill-switch is a coarse emergency tool; a few seconds of skew around the disable moment is tolerable. Documented as a Known limitation. No code change. A future tightening = persist a server `received_at` column + filter on it (Alembic 0006).
- [x] [Review][Decision] **(clou-3+clou-4) Unlocked cache check-await-set** → **RESOLVED: add `asyncio.Lock` to both.** `_DegradedCache.get` and `AlertClassFilter._load` now serialise the check-await-set with double-checked locking (single query on a cold miss). `AlertClassFilter` also bumps a `_generation` on `invalidate()` so a load whose query overlapped an admin write does NOT cache its stale snapshot (kill-switch can't be masked for a TTL). +2 concurrency tests. [cloud-backend/src/cloud_backend/routes/health.py · services/fanout_filter.py]

**Verification (2026-06-13):** cloud-backend unit 86 passed (was 83; +3 new), mypy clean on both touched files (pre-existing `preferences.py` baseline errors untouched per surgical rule + dev R2 note), ruff clean.
...
- **Kill-switch clock-skew (review R3 — clou-1, accepted PoC):** the fan-out filter compares onboard-clock `t_raised` against server-clock `disabled_at`. A train whose clock is skewed vs the cloud can leak a genuinely-post-disable alert (clock slow) or briefly over-suppress an in-flight escalation (clock fast) around the disable moment. **Depends on an NTP-synced fleet.** A future tightening persists a server `received_at` column (Alembic 0006) and filters on it.
